...
assert.match(server, /\['paid', 'fulfilled'\]\.includes\(String\(order\.status\)\)/, "unpaid orders must fail closed");
...
assert.match(library, /Licensed Beats/, "vNext Library must expose the licensed-beat workspace");
...
console.log("vNext licensed-beat Song Workspace checks passed");
