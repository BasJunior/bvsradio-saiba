...
assert(appExperience.includes(':is(input, textarea, select)') && appExperience.includes('font-size: 16px !important'), 'contained iPhone form controls must stay at 16px or larger so WKWebView does not auto-zoom Pulse/chat inputs')
...
console.log('Share overlay, hero clipping, fixed chrome, iOS input zoom guard, Now Playing top edge and purchase handoff assertions passed.')
