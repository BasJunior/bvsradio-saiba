...
const stationPlayer = read('src/components/StationPlayer.tsx')
...
assert(appExperienceStyle.includes('left: max(.7rem, env(safe-area-inset-left)) !important'), 'native mini-player must float with the same side inset as the tab pill')
assert(appExperienceStyle.includes('border-radius: 1.2rem !important'), 'native mini-player must be a rounded capsule')
assert(appExperienceStyle.includes('var(--bvs-nav-height, var(--bvs-app-bottom-nav-height)) + .7rem'), 'native mini-player must clear the measured tab bar')
...
assert(nowPlayingBridge.includes('__bvsNativeMediaQueue'), 'native commands must queue while the React bridge is not ready')
assert(nowPlayingBridge.includes('command === "seek"'), 'native bridge must handle absolute seek commands')
assert(nowPlayingBridge.includes('command === "skip-forward"'), 'native bridge must handle forward skip commands')
assert(nowPlayingBridge.includes('canNext'), 'native controls must reflect real BVS queue availability')
assert(nowPlayingBridge.includes('canSeek'), 'native controls must reflect real seek availability')
assert(!nowPlayingBridge.includes('}, [player]);'), 'native media listener must not detach on every player render')
