...
Path('src/components/editorial/EditorialConnectedPreview.tsx').write_text(r'''\
'use client'
...
Path('src/components/editorial/EditorialEditableIdentity.tsx').write_text(r'''\
'use client'
...
replace_exact('src/app/admin/editorial/page.tsx', "import { creatorPublicName } from '@/lib/public-name'", "import { creatorPublicName, producerPublicName } from '@/lib/public-name'")
replace_exact('src/app/admin/editorial/page.tsx', "type Release = { id: string; title: string; artist_name: string;", "type Release = { id: string; user_id: string; title: string; artist_name: string;")
replace_exact('src/app/admin/editorial/page.tsx', "creator_name_reviewed_at?: string; onboarding_artist_name?: string;", "creator_name_reviewed_at?: string; producer_public_name?: string; producer_name_status?: string; onboarding_artist_name?: string;")
replace_exact('src/app/admin/editorial/page.tsx', "          knownIsrcMap={data.knownIsrcMap || []}\n          canApprove={allowed('approve_submissions')}", "          knownIsrcMap={data.knownIsrcMap || []}\n          profiles={data.profiles}\n          canApprove={allowed('approve_submissions')}")
replace_exact(
    'src/app/admin/editorial/page.tsx',
    "  const nameFor = (id: string) =>\n    profiles.find((p) => p.id === id)?.display_name ||\n    profiles.find((p) => p.id === id)?.username ||\n    id.slice(0, 8)",
    "  const nameFor = (id: string) => {\n    const profile = profiles.find((p) => p.id === id)\n    if (!profile) return id.slice(0, 8)\n    return producerPublicName({ producerPublicName: profile.producer_public_name, producerNameStatus: profile.producer_name_status, publicName: profile.creator_public_name, publicNameStatus: profile.creator_name_status, username: profile.username })\n  }",
)
replace_exact(
    'src/app/admin/editorial/page.tsx',
    ...
    "function TrackCard({ track, credits, messages, profiles, allowed, act, busy }: { track: Track; credits: Credit[]; messages: TrackReviewMessage[]; profiles: Profile[]; allowed: (p: EditorialPermission) => boolean; act: (action: string, body: Record<string, unknown>) => Promise<void>; busy: string }) {",
)
replace_exact(
    'src/app/admin/editorial/page.tsx',
    ...
    "<div className=\"min-w-0 flex-1\"><p className={`text-xs font-semibold uppercase tracking-wider ${statusClass[track.editorial_status] || 'text-text-secondary'}`}>{track.editorial_status.replace('_', ' ')}</p><div className=\"mt-1\"><EditorialEditableIdentity kind=\"track\" id={track.id} title={track.title} artist={track.artist_name} currentProfileId={track.user_id} profiles={profiles} editable={allowed('approve_submissions')} secondary={`${track.genre} · ${new Date(track.created_at).toLocaleDateString()}`} /></div><p className=\"mt-2 text-xs text-text-secondary\">{track.artwork_url?'Submitted artwork attached':'Request artwork before publishing if required.'}</p></div></div><div className=\"min-w-[16rem] flex-1 sm:max-w-md\"><EditorialConnectedPreview previewId={`single:${track.id}`} title={track.title} artist={track.artist_name} src={track.file_url} artwork={track.artwork_url} project=\"Editorial · Singles\" genre={track.genre} compact /></div></div>",
)
replace_exact('src/app/admin/editorial/page.tsx', "                profile={profiles.find((profile) => profile.id === track.user_id)}\n                allowed={allowed}", "                profiles={profiles}\n                allowed={allowed}")
replace_exact(
    'src/app/admin/editorial/page.tsx',
    ...
    "                        <div className=\"mt-1\"><EditorialEditableIdentity kind=\"beat\" id={beat.id} title={beat.title} artist={nameFor(beat.producer_user_id)} currentProfileId={beat.producer_user_id} profiles={profiles} editable={enabled} secondary={`${beat.genre || 'Beat'}${beat.bpm ? ` · ${beat.bpm} BPM` : ''}${price != null ? ` · $${Number(price).toFixed(2)} standard lease` : ''}${beat.is_public ? ' · public' : ' · not public'}`} /></div>",
)
replace_exact('src/app/admin/editorial/page.tsx', "                    {audioSrc ? <audio controls preload=\"none\" src={audioSrc} className=\"h-10 max-w-full\" /> : null}", "                    {audioSrc ? <div className=\"min-w-[16rem] flex-1 sm:max-w-md\"><EditorialConnectedPreview previewId={`beat:${beat.id}`} title={beat.title} artist={nameFor(beat.producer_user_id)} src={audioSrc} artwork={artSrc} project=\"Editorial · BeatStore\" genre={beat.genre} compact /></div> : null}")

replace_exact('src/components/ReleaseEditorialPanel.tsx', "import EditorialSectionCarousel, { EditorialArtistGroupCard, groupEditorialByArtist, matchesEditorialFilter } from '@/components/EditorialSectionCarousel'", "import EditorialSectionCarousel, { EditorialArtistGroupCard, groupEditorialByArtist, matchesEditorialFilter } from '@/components/EditorialSectionCarousel'\nimport EditorialConnectedPreview from '@/components/editorial/EditorialConnectedPreview'\nimport EditorialEditableIdentity, { EditorialReleaseTrackTitleEditor, type EditorialIdentityProfile } from '@/components/editorial/EditorialEditableIdentity'")
replace_exact('src/components/ReleaseEditorialPanel.tsx', "type Release = {\n  id: string\n  title: string", "type Release = {\n  id: string\n  user_id: string\n  title: string")
replace_exact('src/components/ReleaseEditorialPanel.tsx', "  knownIsrcMap = [],\n  canApprove,", "  knownIsrcMap = [],\n  profiles,\n  canApprove,")
replace_exact('src/components/ReleaseEditorialPanel.tsx', "  knownIsrcMap?: KnownIsrcEntry[]\n  canApprove: boolean", "  knownIsrcMap?: KnownIsrcEntry[]\n  profiles: EditorialIdentityProfile[]\n  canApprove: boolean")
replace_exact(
...
replace_exact('src/components/ReleaseEditorialPanel.tsx', "                          <span className=\"min-w-0 flex-1\">{m.position}. {m.title}</span>", "                          <span className=\"shrink-0\">{m.position}.</span><EditorialReleaseTrackTitleEditor releaseId={release.id} releaseTrackId={m.id} releaseTitle={release.title} releaseArtist={release.artist_name} currentProfileId={release.user_id} title={m.title} editable={canApprove} />")
replace_exact('src/components/ReleaseEditorialPanel.tsx', "                        {m.file_url && (\n                          <audio controls preload=\"none\" src={m.file_url} className=\"mt-1 h-8 max-w-full\" />\n                        )}", "                        {m.file_url && (\n                          <div className=\"mt-2\"><EditorialConnectedPreview previewId={`release-track:${m.id}`} title={m.title} artist={release.artist_name} src={m.file_url} artwork={release.cover_url} project={`Editorial · ${release.title}`} genre={release.genre} compact /></div>\n                        )}")

replace_exact('src/components/EditorialWorkDrawer.tsx', "import EditorialAmuseQueue, { PRIVATE_DSP_PARTNER_AMUSE, partnerHandoffNotes } from '@/components/EditorialAmuseQueue'", "import EditorialAmuseQueue, { PRIVATE_DSP_PARTNER_AMUSE, partnerHandoffNotes } from '@/components/EditorialAmuseQueue'\nimport EditorialConnectedPreview from '@/components/editorial/EditorialConnectedPreview'")
replace_exact('src/components/EditorialWorkDrawer.tsx', "                    {work.audio ? <audio controls preload=\"none\" src={work.audio} className=\"mt-4 h-10 w-full max-w-full\" /> : null}", "                    {work.audio ? <div className=\"mt-4\"><EditorialConnectedPreview previewId={`work:${work.kind}:${work.id}`} title={work.title} artist={work.subtitle || 'BVS submission'} src={work.audio} artwork={work.artwork} project=\"Editorial work review\" /></div> : null}")

replace_exact('src/app/api/admin/editorial/catalogue-metadata/route.ts', "import { creatorPublicName } from '@/lib/public-name'", "import { creatorPublicName, producerPublicName } from '@/lib/public-name'")
replace_exact('src/app/api/admin/editorial/catalogue-metadata/route.ts', "  is_published?: boolean | null\n}", "  is_published?: boolean | null\n  producer_public_name?: string | null\n  producer_name_status?: string | null\n}")
replace_exact('src/app/api/admin/editorial/catalogue-metadata/route.ts', "`profiles?id=eq.${encodeURIComponent(profileId)}&select=id,username,display_name,creator_public_name,creator_name_status,role,is_producer,is_published&limit=1`,", "`profiles?id=eq.${encodeURIComponent(profileId)}&select=id,username,display_name,creator_public_name,creator_name_status,producer_public_name,producer_name_status,role,is_producer,is_published&limit=1`,")
replace_exact('src/app/api/admin/editorial/catalogue-metadata/route.ts', "  kind?: 'track' | 'release'", "  kind?: 'track' | 'release' | 'beat'")
replace_exact('src/app/api/admin/editorial/catalogue-metadata/route.ts', "  if (!id || !title || !['track', 'release'].includes(String(kind))) {", "  if (!id || !title || !['track', 'release', 'beat'].includes(String(kind))) {")
marker = "    if (kind === 'track') {"
beat_branch = r'''    if (kind === 'beat') {
...
'''
replace_exact('src/app/api/admin/editorial/catalogue-metadata/route.ts', marker, beat_branch + marker)

Path('scripts/editorial-connected-playback-tests.mjs').write_text(r'''\
...
scripts['test:editorial-connected'] = 'node scripts/editorial-connected-playback-tests.mjs'
for key in ('build', 'vercel-build'):
    command = scripts[key]
    if 'test:editorial-connected' not in command:
        command = command.replace('npm run test:editorial-normalization &&', 'npm run test:editorial-normalization && npm run test:editorial-connected &&')
    scripts[key] = command
Path('package.json').write_text(json.dumps(package, indent=2) + '\n')

Path('.github/workflows/editorial-connected-playback-patch.yml').unlink(missing_ok=True)
Path('scripts/one-shot-editorial-connected-patch.py').unlink(missing_ok=True)
