    ...
    <form id={formId} onSubmit={submit} className="rounded-xl border border-white/10 p-4 sm:p-5">
      ...
      {current?.currentArtwork && (
        <img
          ...
          className="mt-3 h-24 w-24 rounded-lg object-cover border border-white/10"
        />
      )}
      <select
        value={requestType}
        onChange={(event) => setRequestType(event.target.value)}
        className={`${field} mt-3 min-h-11 text-base`}
      >
      ...
      {requestType === "artwork_replacement" && (
        <label className="mt-3 block text-sm text-text-secondary">
          ...
          <input
            ...
            className={`${field} mt-1 min-h-11 text-base`}
          />
          <span className="mt-1 block text-xs">JPG, PNG or WebP · maximum 8MB.</span>
        </label>
      )}
      ...
      <textarea
        ...
        placeholder="Tell editorial what should change."
        className={`${field} mt-3 min-h-28 text-base`}
      />
      <button
        disabled={busy || !targets.length}
        className="mt-3 min-h-11 rounded-full bg-brand px-5 py-2 font-semibold text-black disabled:opacity-40"
      >
