...
export default function StudioMoneySummary({ token }: { token: string }) {
  ...
  return (
    <div className="space-y-10 py-2">
      <CreatorMusicValue token={token} embedded />
      ...
      <section className="border-t border-white/10 pt-8">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            <Link href="/creator/money" className="rounded-full border border-brand/40 bg-brand/10 px-4 py-2 text-sm font-medium text-brand hover:bg-brand/15">
              Open Rights + Money
            </Link>
            <Link href="/artists" className="rounded-full border border-white/20 px-4 py-2 text-sm hover:border-brand">
              Full wallet & payouts
            </Link>
          </div>
        </div>

        {loading && <p className="py-6 text-sm text-text-secondary">Loading marketplace wallet detail…</p>}

        {error && (
          <p className="mt-5 rounded-xl border border-amber-300/25 bg-amber-300/10 p-4 text-sm text-amber-100">
            {error}
          </p>
        )}
                ...
                {latest ? (
                  <div className="mt-3 space-y-2 text-sm text-text-secondary">
                    <p>
                      ...
                      {latest.platform_fee_bps != null ? ` · BVS fee ${((Number(latest.platform_fee_bps) || 0) / 100).toFixed((Number(latest.platform_fee_bps) || 0) % 100 === 0 ? 0 : 1)}%` : ""}
                    </p>
