...
type Props = { trackId?: string; beatId?: string; compact?: boolean };

export default function AppPlaylistPicker({ trackId, beatId, compact = false }: Props) {
  ...
  if (!signedIn || (!canonicalTrackId && !canonicalBeatId)) return null;
  ...
  const add = async (playlist: AppPlaylist) => {
    ...
    if (response?.ok) {
      ...
      setMessage(payload?.error || `Could not add this ${canonicalBeatId ? "beat" : "track"}.`);
    }
    setBusyId("");
  };
