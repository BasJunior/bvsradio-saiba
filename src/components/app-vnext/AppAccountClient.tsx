...
type RoleApplication = { id: string; requested_role: "artist" | "producer" | "writer" | "show_creator"; status: "submitted" | "information_requested" | "approved" | "rejected"; message?: string; review_notes?: string; updated_at?: string };
...
export default function AppAccountClient({ surface }: { surface: AppSurface }) {
  ...
  const [application, setApplication] = useState<RoleApplication | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setData(account);
      setApplication(applicationPayload.application || null);
      setForm({
  ...
  const photo = avatarPreview || data.profile.display_avatar_url || data.profile.avatar_url || "/assets/images/default-avatar.png";
  return <div className="mx-auto max-w-5xl px-4 pb-12 pt-6 sm:px-6">
    ...
    <form onSubmit={save} className="mt-7 rounded-[1.75rem] border border-white/10 bg-white/[.025] p-5">
      <div className="flex flex-wrap items-center gap-4"><div className="relative h-20 w-20 overflow-hidden rounded-full border border-white/10"><Image src={photo} alt="Profile" fill unoptimized className="object-cover" /></div><div className="min-w-0 flex-1"><h2 className="text-2xl font-semibold">Profile</h2><p className="mt-1 text-sm text-text-secondary">Your public BVS identity and private account name.</p><label className="mt-3 inline-flex cursor-pointer rounded-full border border-white/15 px-4 py-2 text-sm">Change photo<input type="file" accept="image/*" className="hidden" onChange={(event) => chooseAvatar(event.target.files?.[0] || null)} /></label></div></div>
      <div className="mt-6 grid gap-4 sm:grid-cols-2"><label className="text-sm">Username<input value={form.username} readOnly className={`${inputClass} opacity-65`} /></label><label className="text-sm">Display name<input value={form.displayName} onChange={(event) => setForm((current) => ({ ...current, displayName: event.target.value }))} maxLength={100} className={inputClass} /></label><label className="text-sm">Full/legal name <span className="text-text-secondary">· private</span><input value={form.fullName} onChange={(event) => setForm((current) => ({ ...current, fullName: event.target.value }))} maxLength={160} className={inputClass} /></label>{access?.artist ? <label className="text-sm">Artist public name<input value={form.creatorPublicName} onChange={(event) => setForm((current) => ({ ...current, creatorPublicName: event.target.value }))} maxLength={120} className={inputClass} /></label> : null}{access?.producer ? <label className="text-sm">Producer public name<input value={form.producerPublicName} onChange={(event) => setForm((current) => ({ ...current, producerPublicName: event.target.value }))} maxLength={120} placeholder="Leave blank to keep current setting" className={inputClass} /></label> : null}</div><label className="mt-4 block text-sm">Bio<textarea value={form.bio} onChange={(event) => setForm((current) => ({ ...current, bio: event.target.value }))} maxLength={1000} rows={4} className={inputClass} /></label><div className="mt-5 flex flex-wrap items-center gap-2"><button type="submit" disabled={busy} className="min-h-11 rounded-full bg-brand px-5 text-sm font-semibold text-black disabled:opacity-50">{busy ? "Saving…" : "Save profile"}</button>{data.profile.is_verified ? <span className="rounded-full border border-brand/30 px-3 py-2 text-xs text-brand">Verified</span> : null}<span className="rounded-full border border-white/10 px-3 py-2 text-xs text-text-secondary">{data.profile.role || "listener"}{data.profile.is_producer ? " · producer" : ""}</span></div>
    </form>

    <section id="creator-role" className="mt-7 rounded-[1.75rem] border border-white/10 bg-white/[.025] p-5"><p className="text-xs uppercase tracking-[.18em] text-brand">Creator access</p><h2 className="mt-1 text-2xl font-semibold">Grow this account into another role.</h2>{application ? <div className="mt-4 rounded-2xl border border-white/10 bg-black/10 p-4"><div className="flex flex-wrap items-center justify-between gap-2"><p className="font-semibold capitalize">{application.requested_role.replaceAll("_", " ")}</p><span className="rounded-full border border-white/10 px-3 py-1 text-xs capitalize">{application.status.replaceAll("_", " ")}</span></div>{application.review_notes ? <p className="mt-2 text-sm text-text-secondary">Editorial: {application.review_notes}</p> : null}</div> : null}<form onSubmit={applyForRole} className="mt-4"><div className="grid gap-3 sm:grid-cols-[.45fr_1fr]"><label className="text-sm">Role<select value={roleForm.requestedRole} onChange={(event) => setRoleForm((current) => ({ ...current, requestedRole: event.target.value }))} className={inputClass}><option value="artist">Artist</option><option value="producer">Producer</option><option value="writer">Writer</option><option value="show_creator">Show creator</option></select></label><label className="text-sm">Tell editorial about your work<textarea value={roleForm.message} onChange={(event) => setRoleForm((current) => ({ ...current, message: event.target.value }))} minLength={20} maxLength={2000} rows={3} className={inputClass} /></label></div><button type="submit" disabled={busy || roleForm.message.trim().length < 20} className="mt-4 min-h-11 rounded-full border border-brand/35 bg-brand/10 px-5 text-sm font-semibold text-brand disabled:opacity-40">Submit role application</button></form>{isCreator ? <Link href={`/app/${surface}/studio`} className="mt-4 inline-flex text-sm text-brand">Open BVS Studio →</Link> : null}</section>

    <section className="mt-7 rounded-[1.75rem] border border-white/10 bg-white/[.025] p-5"><p className="text-xs uppercase tracking-[.18em] text-brand">Privacy & control</p><h2 className="mt-1 text-2xl font-semibold">Your data stays manageable.</h2><div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={() => void exportData()} className="min-h-11 rounded-full border border-white/15 px-5 text-sm">Export my data</button><button type="button" onClick={() => setShowDeletion((value) => !value)} className="min-h-11 rounded-full border border-red-400/30 px-5 text-sm text-red-200">Delete account</button></div>{showDeletion ? <div className="mt-5 rounded-2xl border border-red-400/25 bg-red-500/[.06] p-4"><h3 className="font-semibold text-red-100">Request account deletion</h3><p className="mt-2 text-sm text-text-secondary">Type DELETE to confirm. Published creator content or open orders may require fulfilment review before removal.</p><input value={deletionConfirmation} onChange={(event) => setDeletionConfirmation(event.target.value)} placeholder="DELETE" className={inputClass} /><textarea value={deletionReason} onChange={(event) => setDeletionReason(event.target.value)} maxLength={1000} rows={3} placeholder="Optional reason" className={inputClass} /><button type="button" disabled={busy || deletionConfirmation.trim().toUpperCase() !== "DELETE"} onClick={() => void requestDeletion()} className="mt-3 min-h-11 rounded-full bg-red-500 px-5 text-sm font-semibold text-white disabled:opacity-40">Submit deletion request</button></div> : null}</section>
  </div>;
}
