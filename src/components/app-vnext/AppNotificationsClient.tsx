...
export default function AppNotificationsClient({ surface }: { surface: AppSurface }) {
  ...
  if (!signedIn) return (
    <div className="mx-auto max-w-3xl px-4 pb-12 pt-10 text-center sm:px-6">
      ...
      <Link href={`/auth/login?next=${encodeURIComponent(`/app/${surface}/notifications`)}`} className="mt-7 inline-flex min-h-11 items-center rounded-full bg-white px-6 font-semibold text-black transition hover:bg-brand">Sign in</Link>
    </div>
  );
