...
export default function AppOfflineDownloads({ surface }: { surface: AppSurface }) {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("bvs:offline-change", sync);
  }, [refresh]);
  ...
  return (
    <section className="mt-9">
      ...
      <p className="mt-2 max-w-2xl text-sm text-text-secondary">Only recordings with explicit {surface === "ios" ? "iOS" : "Android"} clearance can be stored. Files stay inside BVS and periodically revalidate their rights.</p>
      {!offlineMediaAvailable() ? (
            ...
            return (
              <article key={item.trackId} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-white/10 bg-white/[.025] p-4">
                ...
                <div className="flex gap-2">
                  {!valid ? <button type="button" onClick={() => void renew(item)} className="min-h-10 rounded-full border border-brand/40 px-3 text-xs font-semibold text-brand">Renew</button> : null}
