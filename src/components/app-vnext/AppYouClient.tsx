...
export default function AppYouClient({ surface }: { surface: AppSurface }) {
  ...
  if (!signedIn) return <div className="mx-auto max-w-4xl px-4 pb-10 pt-8 text-center sm:px-6"><p className="text-xs uppercase tracking-[.2em] text-brand">You</p><h1 className="mt-2 text-4xl font-semibold">Make BVS yours.</h1><p className="mx-auto mt-4 max-w-xl text-text-secondary">Join to sync your library, follow creators and grow the same account into a Studio identity later.</p><Link href={`/app/${surface}/join`} className="mt-7 inline-flex min-h-11 items-center rounded-full bg-brand px-6 font-semibold text-black">Join BVS</Link><div className="mt-3"><Link href={`/auth/login?next=${encodeURIComponent(`/app/${surface}/you`)}`} className="text-sm text-brand">Already a member? Sign in</Link></div><div className="mt-6 flex flex-wrap justify-center gap-2 text-sm"><a href="https://bvsradio.com/contact" target="_blank" rel="noopener noreferrer" className="rounded-full border border-white/15 px-4 py-2 text-text-secondary hover:border-brand/40">Support ↗</a><a href="https://bvsradio.com/privacy" target="_blank" rel="noopener noreferrer" className="rounded-full border border-white/15 px-4 py-2 text-text-secondary hover:border-brand/40">Privacy ↗</a></div></div>;
