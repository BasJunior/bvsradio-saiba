...
export default function AppYouClient({ surface }: { surface: AppSurface }) {
  ...
  useEffect(() => {
    try {
      const saved = window.localStorage.getItem("bvs_app_data_mode") as DataMode | null;
      if (saved === "auto" || saved === "saver" || saved === "high") setDataMode(saved);
      setWifiOnly(window.localStorage.getItem("bvs_app_wifi_downloads") !== "0");
    } catch {}
  }, []);

  const updateMode = (next: DataMode) => {
    setDataMode(next);
    try { window.localStorage.setItem("bvs_app_data_mode", next); } catch {}
    window.dispatchEvent(new CustomEvent("bvs:app-data-mode", { detail: { mode: next } }));
  };
  const updateWifi = (next: boolean) => {
    setWifiOnly(next);
    try { window.localStorage.setItem("bvs_app_wifi_downloads", next ? "1" : "0"); } catch {}
  };
  const signOut = async () => {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 pb-10 pt-6 sm:px-6">
      ...
      <section className="mt-7"><p className="text-xs uppercase tracking-[.18em] text-brand">Listening data</p><h2 className="mt-1 text-2xl font-semibold">Control how BVS uses your connection.</h2><div className="mt-4 grid gap-2 sm:grid-cols-3">{(["auto", "saver", "high"] as DataMode[]).map((mode) => <button key={mode} type="button" onClick={() => updateMode(mode)} className={`rounded-2xl border p-4 text-left ${dataMode === mode ? "border-brand/45 bg-brand/10" : "border-white/10 bg-white/[.02]"}`}><span className="block font-semibold capitalize">{mode === "saver" ? "Data Saver" : mode === "high" ? "High quality" : "Auto"}</span><span className="mt-1 block text-xs text-text-secondary">{mode === "saver" ? "Use less data and lighter media." : mode === "high" ? "Prefer richer media when the network allows." : "Let BVS adapt to the connection."}</span></button>)}</div><label className="mt-3 flex items-center justify-between gap-4 rounded-2xl border border-white/10 p-4"><span><span className="block font-semibold">Downloads on Wi-Fi only</span><span className="mt-1 block text-xs text-text-secondary">Foundation setting for the upcoming rights-controlled offline store.</span></span><input type="checkbox" checked={wifiOnly} onChange={(event) => updateWifi(event.target.checked)} className="h-5 w-5 accent-brand" /></label></section>

      <section className="mt-7 grid gap-2 sm:grid-cols-2"><Link href="/notifications" className="rounded-2xl border border-white/10 p-4"><h2 className="font-semibold">Notifications</h2><p className="mt-1 text-sm text-text-secondary">Editorial, creator, order and account updates.</p></Link><Link href="/account" className="rounded-2xl border border-white/10 p-4"><h2 className="font-semibold">Account Centre</h2><p className="mt-1 text-sm text-text-secondary">Profile, roles, data export and account deletion.</p></Link>{isCreator ? <Link href={`/app/${surface}/studio`} className="rounded-2xl border border-brand/25 bg-brand/[.05] p-4"><h2 className="font-semibold">BVS Studio</h2><p className="mt-1 text-sm text-text-secondary">Open your creator workspace.</p></Link> : <Link href="/account" className="rounded-2xl border border-brand/25 bg-brand/[.05] p-4"><h2 className="font-semibold">Become a creator</h2><p className="mt-1 text-sm text-text-secondary">Add a creator role to this account.</p></Link>}</section>
