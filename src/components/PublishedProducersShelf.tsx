...
export default function PublishedProducersShelf({ onBrowse }: { onBrowse: (producer: string) => void }) {
  ...
  if (!producers.length) return <p className="rounded-xl border border-dashed border-white/10 p-5 text-sm text-text-secondary">Published producers will appear here after their first beat is approved and published.</p>
