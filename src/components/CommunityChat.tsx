...
export default function CommunityChat({
  ...
  const refresh = useCallback(async (quiet = false) => {
    ...
    try {
      const response = await fetch(`/api/community/messages?room=${encodeURIComponent(roomId)}`, { headers: { Authorization: `Bearer ${accessToken}` }, cache: 'no-store' })
  ...
  useEffect(() => {
    if (roomId === 'bvs-live') return
    const started = Date.now()
    trackEvent('show_room_enter', { room_id: roomId })
    const thirty = window.setTimeout(() => trackEvent('show_room_30s', { room_id: roomId }), 30_000)
    const fiveMinutes = window.setTimeout(() => trackEvent('show_room_5m', { room_id: roomId }), 300_000)
    return () => {
      window.clearTimeout(thirty)
      window.clearTimeout(fiveMinutes)
      trackEvent('show_room_exit', { room_id: roomId, seconds: Math.round((Date.now() - started) / 1000) })
    }
  }, [roomId])
  async function submit(event: FormEvent) { event.preventDefault(); if (!message.trim() || sending) return; const accessToken = await token(); if (!accessToken) return setError('Sign in to post.'); setSending(true); setError(''); setNotice(''); try { const response = await fetch('/api/community/messages', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${accessToken}` }, body: JSON.stringify({ message, room: roomId }) }); const payload = await response.json(); if (!response.ok) throw new Error(payload.error || 'Could not post message.'); setMessage(''); await refresh(true) } catch (issue) { setError(issue instanceof Error ? issue.message : 'Could not post message.') } finally { setSending(false) } }
