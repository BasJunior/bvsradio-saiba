...
type OfflinePlugin={download(options:{manifest:OfflineManifest}):Promise<{item:OfflineItem}>;list():Promise<{items:OfflineItem[]}>;remove(options:{trackId:string}):Promise<void>;status(options:{trackId:string}):Promise<{item:OfflineItem|null}>;renew(options:{manifest:OfflineManifest}):Promise<{item:OfflineItem}>;playbackSource(options:{trackId:string}):Promise<{item:OfflineItem;uri:string}>};
...
export async function offlinePlaybackSource(trackId:string){if(!isNativeRuntime())throw new Error("Offline playback requires the installed BVS app.");const result=await BvsOfflineMedia.playbackSource({trackId});if(!result?.uri)throw new Error("The private offline file is unavailable.");return{item:result.item,src:Capacitor.convertFileSrc(result.uri)}}
