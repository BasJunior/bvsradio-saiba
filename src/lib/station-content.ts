...
export async function getPublicProgrammes(): Promise<Show[]> {
  ...
  try {
    const response = await fetch(`${url}/rest/v1/programmes?status=in.(scheduled,active)&select=*&order=day_label,start_time`, { headers: { apikey: key, Authorization: `Bearer ${key}` }, next: { revalidate: 60 }, signal: AbortSignal.timeout(4000) })
...
export async function getPublicShowEvent(programmeSlug: string): Promise<ShowEvent | null> {
  ...
  try {
    ...
    const response = await fetch(
      ...
      { headers: publicHeaders(supabase.key), next: { revalidate: 30 }, signal: AbortSignal.timeout(4000) },
    )
  ...
  try {
    const [creatorsResponse, setlistResponse] = await Promise.all([
      fetch(
        ...
        { headers, next: { revalidate: 30 }, signal: AbortSignal.timeout(4000) },
      ),
      fetch(
        ...
        { headers, next: { revalidate: 15 }, signal: AbortSignal.timeout(4000) },
      ),
    ])
    ...
    if (profileIds.length) {
      const profilesResponse = await fetch(
        ...
        { headers, next: { revalidate: 60 }, signal: AbortSignal.timeout(4000) },
      )
