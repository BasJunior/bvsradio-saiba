...
export function marketplaceCommissionBps(input: CommissionPolicyInput): number | null {
  ...
  if (input.productType === 'single' || input.productType === 'mix') {
    if (plan.includes('artist_founding') || plan.includes('artist_standard') || plan.includes('artist_plus') || plan.includes('creator_complete')) return 1500
    return 2000
  }
...
export function processorFeeFromPreset(totalCharged: number, presetId: string): number | null {
  ...
  return Math.round((amount * (preset.percent / 100) + preset.fixed) * 100) / 100
}
...
export function calculateMarketplaceEconomics(input: MarketplaceCalculationInput): MarketplaceCalculation {
  ...
  const processorAllocatedToSeller = Math.max(0, Number(input.processorAllocatedToSeller ?? processorFee) || 0)
  const commission = Math.round(productPrice * (Math.max(0, input.commissionBps) / 10000) * 100) / 100
  const tax = Math.round(productPrice * taxRate * 100) / 100
  const sellerNet = Math.max(0, Math.round((productPrice - commission - processorAllocatedToSeller) * 100) / 100)
  const bvsContributionAfterProcessing = Math.round((commission - Math.max(0, processorFee - processorAllocatedToSeller)) * 100) / 100
...
export function producerUpgradeBreakEvenUsd(monthlyPrice: number, commissionSavingsPercent: number) {
  if (monthlyPrice <= 0 || commissionSavingsPercent <= 0) return null
  return Math.round((monthlyPrice / (commissionSavingsPercent / 100)) * 100) / 100
}
