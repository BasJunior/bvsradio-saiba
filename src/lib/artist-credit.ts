...
async function policyForOrderLine(sellerId: string, item: CommerceItemRow) {
  ...
  if (
    Number.isFinite(frozenBps) &&
    frozenBps >= 0 &&
    item.seller_plan_id_snapshot
  ) {
    return {
      planId: String(item.seller_plan_id_snapshot),
      bps: Math.floor(frozenBps),
      policyVersion: String(
        item.marketplace_policy_version_snapshot || MARKETPLACE_POLICY_VERSION,
      ),
...
export async function creditPaidArtistDeposit(
  reference: string,
  source: "stripe" | "paynow",
) {
  if (!url || !service || !reference)
    return { credited: false, reason: "not_configured" };
  const orderResponse = await fetch(
    `${url}/rest/v1/orders?reference=eq.${encodeURIComponent(reference)}&select=id,reference,customer_user_id,items&limit=1`,
    { headers, cache: "no-store" },
  );
  if (!orderResponse.ok)
    return { credited: false, reason: "order_lookup_failed" };
  const [order] = (await orderResponse.json()) as OrderRow[];
  if (!order?.customer_user_id)
    return { credited: false, reason: "anonymous_order" };
  const amount = (order.items || [])
    .filter((item) => item.type === "artist_deposit")
    .reduce(
      (sum, item) =>
  ...
  const depositResponse = await fetch(
    `${url}/rest/v1/artist_deposits?on_conflict=source,external_reference`,
    {
      method: "POST",
      headers: {
        ...headers,
      ...
      body: JSON.stringify({
        artist_user_id: order.customer_user_id,
        amount,
        currency: "USD",
        status: "credited",
        source,
        ...
        creditable_at: new Date().toISOString(),
        credited_at: new Date().toISOString(),
      }),
    },
  );
  if (!depositResponse.ok)
    return { credited: false, reason: "deposit_save_failed" };
  ...
  const existingResponse = await fetch(
    `${url}/rest/v1/artist_ledger_entries?deposit_id=eq.${deposit.id}&entry_type=eq.deposit_credit&select=id&limit=1`,
    { headers, cache: "no-store" },
  );
  if (existingResponse.ok && (await existingResponse.json()).length)
    return { credited: true, idempotent: true };
  const ledgerResponse = await fetch(`${url}/rest/v1/artist_ledger_entries`, {
    ...
    body: JSON.stringify({
      ...
      memo: `Creditable BVS artist deposit ${reference}`,
      metadata: { reference, source },
    }),
  });
  if (!ledgerResponse.ok)
    return { credited: false, reason: "ledger_save_failed" };
...
export async function creditPaidArtistSales(
  ...
  if (!url || !service || !reference)
    return { credited: false, reason: "not_configured" };
  const orderResponse = await fetch(
    `${url}/rest/v1/orders?reference=eq.${encodeURIComponent(reference)}&status=in.(paid,fulfilled)&select=id,reference&limit=1`,
    { headers, cache: "no-store" },
  );
  if (!orderResponse.ok)
    return { credited: false, reason: "order_lookup_failed" };
  ...
  if (!itemResponse.ok)
    return { credited: false, reason: "items_lookup_failed" };
  const items = (await itemResponse.json()) as CommerceItemRow[];
  const totalProductRevenue = items.reduce(
    (sum, item) =>
      sum + (Number(item.unit_amount) || 0) * (Number(item.quantity) || 0),
    0,
  );
  const sellerItems = items.filter((item) =>
    Boolean(item.seller_user_id_snapshot),
  );
  if (!sellerItems.length)
    return { credited: false, reason: "no_attributed_seller" };
  ...
  for (const [sellerId, rows] of bySeller) {
    ...
    for (const item of rows) {
      const lineGross = roundMoney(
        (Number(item.unit_amount) || 0) * (Number(item.quantity) || 0),
      );
    ...
    if (gross <= 0) continue;
    const processorAllocated =
      processorFee && totalProductRevenue > 0
        ? roundMoney(
            processorFee.amountOrderCurrency * (gross / totalProductRevenue),
          )
        : 0;
    const sellerNet = roundMoney(
      Math.max(0, gross - platformFee - processorAllocated),
    );
    const effectiveBps =
      gross > 0 ? Math.round((platformFee / gross) * 10000) : 0;
    const serviceHeld = rows.every(
      (item) => item.product_type_snapshot === "creator_service",
    );
    const settlementStatus = serviceHeld
      ? "held_service"
      : processorFee
        ? "posted"
        : "pending_processor";
    ...
    const sellerPlanId =
      sellerPlanIds.size === 1 ? [...sellerPlanIds][0] : "mixed";
    const policyVersion =
    ...
    const settlementResponse = await fetch(
      `${url}/rest/v1/commerce_seller_settlements?on_conflict=order_id,seller_user_id`,
      {
        method: "POST",
        headers: {
          ...headers,
        ...
        body: JSON.stringify({
          ...
          breakdown: {
            lines: lineBreakdown,
            processorAllocationBasis: "share_of_pre_tax_order_product_revenue",
          },
    ...
    if (!settlementResponse.ok)
      return { credited: false, reason: "settlement_save_failed" };
    ...
    if (
      ...
      existing.metadata?.policyVersion !== policyVersion
    ) {
    ...
    if (existing?.id) {
      const update = await fetch(
        `${url}/rest/v1/artist_ledger_entries?id=eq.${existing.id}`,
        {
          method: "PATCH",
          headers: { ...headers, Prefer: "return=minimal" },
      ...
      if (!update.ok)
        return { credited: false, reason: "ledger_update_failed" };
    } else {
