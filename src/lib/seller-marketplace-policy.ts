...
function defaultPlan(productType: MarketplaceProductType) {
  if (productType === "beat" || productType === "creator_product")
    return "producer_free";
  if (productType === "service" || productType === "creator_service")
    return "service_free";
  return "artist_free";
}
...
export async function resolveSellerMarketplacePolicy(
  ...
  if (url && service && userId) {
    try {
      ...
      const rows = response.ok
        ? ((await response.json()) as MembershipRow[])
        : [];
      const bundle = rows.find((row) => row.family === "creator_bundle");
      const wantedFamily =
        productType === "beat" || productType === "creator_product"
          ? "producer"
          : productType === "service" || productType === "creator_service"
            ? "service"
            : "artist";
      const selected =
        bundle || rows.find((row) => row.family === wantedFamily);
      if (selected?.plan_id) {
        planId = String(selected.plan_id);
        entitlements =
          selected.entitlements && typeof selected.entitlements === "object"
  ...
  const commissionBps =
    mayOverride && Number.isFinite(entitlementBps) && entitlementBps >= 0
