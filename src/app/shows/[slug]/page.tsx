...
import { getShow, shows } from "@/lib/station";

export function generateStaticParams() { return shows.map(({ slug }) => ({ slug })); }
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> { const show = getShow((await params).slug); return show ? { title: show.title, description: show.description } : {}; }

export default async function ShowPage({ params }: { params: Promise<{ slug: string }> }) {
  const show = getShow((await params).slug); if (!show) notFound();
