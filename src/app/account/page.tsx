...
type Profile = {
  ...
  producer_name_status?: 'not_submitted' | 'pending' | 'approved' | 'changes_requested' | 'rejected'
...
export default function AccountPage() {
  ...
  const [form, setForm] = useState({ username: '', fullName: '', displayName: '', creatorPublicName: '', producerPublicName: '', avatarUrl: '', bio: '' })
  ...
  const load = async (accessToken: string) => {
    ...
    setForm({
      ...
      producerPublicName: account.profile?.producer_name_request || account.profile?.producer_public_name || '',
    ...
    <main className="mx-auto max-w-7xl px-6 py-12">
      ...
      <section className="mt-12 grid gap-8 lg:grid-cols-[1.1fr_.9fr]">
        <form onSubmit={save} className="rounded-2xl border border-white/10 bg-white/[.025] p-6">
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            ...
            {artistCapable && (
              <label className="text-sm font-medium">
                Artist public name
                <input
                  value={form.creatorPublicName}
                  onChange={(event) => setForm({ ...form, creatorPublicName: event.target.value })}
                  placeholder={`@${form.username}`}
                  className={field}
                />
                <span className="mt-1 block text-xs font-normal text-text-secondary">
                  Used on tracks, artist pages and music search after Editorial approval. Until then we show @{form.username}. Dual-role creators may use a different producer name below.
                </span>
              </label>
            )}
            {producerCapable && (
          ...
          {artistCapable && data.profile.creator_name_status && data.profile.creator_name_status !== 'not_submitted' && (
            <div className="mt-5 rounded-xl border border-white/10 bg-black/20 p-4 text-sm">
              <p className="font-medium capitalize">Artist public-name review · {data.profile.creator_name_status.replaceAll('_', ' ')}</p>
              {data.profile.creator_public_name && <p className="mt-1 text-text-secondary">Currently public as artist: {data.profile.creator_public_name}</p>}
          ...
          {producerCapable && data.profile.producer_name_status && data.profile.producer_name_status !== 'not_submitted' && (
            <div className="mt-5 rounded-xl border border-white/10 bg-black/20 p-4 text-sm">
              <p className="font-medium capitalize">Producer public-name review · {data.profile.producer_name_status.replaceAll('_', ' ')}</p>
              {data.profile.producer_public_name && <p className="mt-1 text-text-secondary">Currently public as producer: {data.profile.producer_public_name}</p>}
              {data.profile.producer_name_review_notes && <p className="mt-2 text-text-secondary">Editorial: {data.profile.producer_name_review_notes}</p>}
            </div>
          )}
