  ...
  if (!order) {
    return (
      <main className="mx-auto min-h-[65vh] max-w-xl px-6 py-20 text-center">
        ...
        <Link href="/account" className="mt-6 inline-block text-brand">
          ← Account Centre
        </Link>
      </main>
    )
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-12">
      <div className="print:hidden">
        <Link href="/account" className="text-sm text-brand">
          ← Account Centre
        </Link>
      </div>
      ...
      <section className="mt-6 overflow-hidden rounded-2xl border border-white/10 bg-white/[.025]">
        <header className="flex flex-wrap items-start justify-between gap-5 border-b border-white/10 px-6 py-7 sm:px-9">
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-brand">BVS Radio receipt</p>
            <h1 className="mt-2 font-mono text-2xl font-semibold sm:text-3xl">{order.reference}</h1>
          ...
          <div className="text-right">
            <span
              className={`inline-flex rounded-full border px-3 py-1 text-xs font-semibold capitalize ${statusClass(order.status)}`}
            >
          ...
          <div className="mt-4 space-y-0">
            {order.items.map((item, index) => (
              <div
                ...
                className="flex justify-between gap-5 border-t border-white/10 py-4 first:border-t-0 first:pt-0"
              >
                <div>
                  <p className="font-medium text-white">{item.title || 'BVS item'}</p>
                  <p className="mt-1 text-xs text-text-secondary">
                    {[
                      item.artist,
                      item.type,
                      `Qty ${item.quantity || 1}`,
                      item.price != null ? `${money(item.price, currency)} each` : null,
                    ]
                      .filter(Boolean)
                      .join(' · ')}
                  </p>
          ...
          <dl className="mt-2 space-y-2 border-t border-white/10 pt-5 text-sm">
            ...
            <div className="flex justify-between gap-4">
              <dt className="text-text-secondary">
                {order.tax_label || 'Tax'}
                {order.tax_country ? ` (${order.tax_country})` : ''}
                {typeof order.tax_rate === 'number' && order.tax_rate > 0
                  ? ` · ${(order.tax_rate * 100).toFixed(0)}%`
                  : ''}
              </dt>
              <dd className="tabular-nums">{money(order.tax_amount, currency)}</dd>
            </div>
            <div className="flex justify-between gap-4 text-lg font-semibold">
              <dt>Total</dt>
              <dd className="tabular-nums text-brand">{money(order.total, currency)}</dd>
            </div>
            <div className="flex justify-between gap-4 pt-2">
              <dt className="text-text-secondary">Payment method</dt>
              <dd className="capitalize">{order.payment_method?.replaceAll('_', ' ') || 'Not recorded'}</dd>
            </div>
            <div className="flex justify-between gap-4">
              <dt className="text-text-secondary">Delivery</dt>
              <dd className="capitalize">{order.delivery_status?.replaceAll('_', ' ') || 'Pending'}</dd>
            </div>
          </dl>
        </div>

        {order.licenceSummary?.length > 0 && (
          <section className="border-t border-white/10 px-6 py-6 sm:px-9">
            <h2 className="text-lg font-semibold">Licence record</h2>
            <div className="mt-3 space-y-3">
              {order.licenceSummary.map((item, index) => (
                <div key={`${item.title}-${index}`} className="rounded-xl border border-white/10 bg-black/20 px-4 py-3">
                  <p className="font-medium">{item.title}</p>
                  <p className="mt-1 text-sm leading-relaxed text-text-secondary">{item.licence}</p>
                </div>
              ))}
            </div>
          </section>
        ...
        <section className="border-t border-white/10 px-6 py-6 sm:px-9">
          ...
          {order.downloads.length ? (
            ...
            <p className="mt-2 text-sm leading-relaxed text-text-secondary">
              {paid
                ? 'No automatic file is staged for this item yet. BVS support will complete delivery when ready.'
                : 'Downloads unlock after payment is confirmed.'}
            </p>
          )}
        </section>
