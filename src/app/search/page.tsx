...
type SearchItem = { id: string; kind: SearchKind; title: string; subtitle: string; href: string; image?: string; tags?: string[]; badge?: string }
...
const exploreModes: Array<{ value: ExploreMode; label: string; kinds: SearchKind[] }> = [
  { value: 'fresh', label: 'Fresh', kinds: ['track', 'release', 'beat', 'story'] },
  { value: 'rotation', label: 'On BVS', kinds: ['track', 'release', 'show'] },
  { value: 'creators', label: 'Creators', kinds: ['artist', 'producer'] },
  { value: 'beats', label: 'Beats & Tools', kinds: ['beat', 'service', 'producer'] },
  { value: 'culture', label: 'Shows & Stories', kinds: ['show', 'story'] },
]
...
export default function SearchPage() {
  ...
  return <main className="mx-auto min-h-[70vh] max-w-7xl px-4 py-12 sm:px-6">
    ...
    {!query.trim() && filter === 'all' ? <section className="mt-10" aria-label="Explore published BVS content"><h2 className="text-3xl font-semibold">{flowV2Flags.exploreModes ? exploreModes.find(item => item.value === mode)?.label : 'Discover now'}</h2><p className="mt-2 text-text-secondary">Published and editorially visible content—ordered by current catalogue availability.</p></section> : <h2 className="mt-10 text-3xl font-semibold">{query.trim() ? `Results for “${query.trim()}”` : headings[filter as SearchKind]}</h2>}
