...
export default async function MobileArtistsPage({ params }: { params: Promise<{ surface: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 pb-12 pt-7 sm:px-6">
      ...
      <div className="mt-6">
        <p className="text-xs font-semibold uppercase tracking-[.18em] text-brand">Creators in this edition</p>
        <h1 className="mt-2 text-3xl font-semibold sm:text-4xl">Artists and producers</h1>
        <p className="mt-3 max-w-2xl text-sm leading-6 text-text-secondary">This directory is derived only from the mobile-cleared radio catalogue and rights-confirmed BeatStore listings.</p>
      </div>
      {list.length ? (
