...
export default async function AppBeatPage({ params }: { params: Promise<{ surface: string; id: string }> }) {
  ...
  const producerResponse = await fetch(
    beatUrl(`profiles?id=eq.${encodeURIComponent(beat.producer_user_id)}&select=username,creator_public_name,creator_name_status,producer_public_name,producer_name_status&limit=1`),
    { headers: beatHeaders, cache: "no-store" },
  ).catch(() => null);
  const producerRows = producerResponse?.ok ? await producerResponse.json().catch(() => []) as ProducerProfile[] : [];
  const producer = producerRows[0] || null;
  ...
  return <div className="mx-auto max-w-4xl px-4 pb-12 pt-6 sm:px-6">
    ...
    <section className="mt-5 grid gap-6 sm:grid-cols-[240px,1fr] sm:items-start">
      ...
      <div><p className="text-xs uppercase tracking-[.2em] text-brand">BeatStore · published</p><h1 className="mt-2 text-4xl font-semibold tracking-tight">{beat.title}</h1><p className="mt-2 text-sm text-text-secondary">{producerHandle ? <Link href={`/app/${surface}/creator/${encodeURIComponent(producerHandle)}?as=producer`} className="text-white hover:text-brand">{producerName}</Link> : producerName}</p><p className="mt-3 text-sm text-text-secondary">{[beat.genre, beat.mood, beat.bpm ? `${beat.bpm} BPM` : "", beat.musical_key].filter(Boolean).join(" · ")}</p>{beat.description ? <p className="mt-4 text-sm leading-6 text-text-secondary">{beat.description}</p> : null}<div className="mt-5 flex flex-wrap gap-2"><AppShareButton title={beat.title} text={`${beat.title} by ${producerName} on BVS BeatStore`} path={`/app/${surface}/beat/${beat.id}`} />{!iosCommerceRestricted ? <a href={`https://bvsradio.com/catalogue?type=beat&q=${encodeURIComponent(beat.title)}#beatstore`} target="_blank" rel="noopener noreferrer" className="min-h-10 rounded-full border border-brand/35 px-4 py-2 text-sm font-semibold text-brand">View licence on BVS website</a> : null}</div></div>
    </section>
