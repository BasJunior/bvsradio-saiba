...
export default async function MobileTrackPage({ params }: { params: Promise<{ surface: string; id: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 pb-12 pt-7 sm:px-6">
      <Link href={`${appHome}/explore`} className="text-sm text-brand hover:underline">← Back to Explore</Link>
      <article className="mt-6 overflow-hidden rounded-[2rem] border border-white/10 bg-white/[.025] p-5 sm:p-7">
        <div className="grid gap-6 sm:grid-cols-[12rem_minmax(0,1fr)] sm:items-center">
          ...
          <div className="min-w-0">
            <p className="text-xs font-semibold uppercase tracking-[.18em] text-emerald-200">Cleared recording</p>
        ...
        <p className="mt-4 text-xs leading-5 text-text-secondary">This detail screen uses the same surface-specific cleared response as the mobile radio queue. If the recording is no longer cleared, the route fails closed instead of falling back to the website catalogue.</p>
      </article>
    </div>
  );
}
