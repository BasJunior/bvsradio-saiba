...
const emptyData: WalletData = {
  ...
  earnings: { lifetimeGrossSales: 0, bvsPlatformFees: 0, processorFees: 0, postedNetEarnings: 0, pendingNetEarnings: 0, settlementCount: 0 },
}
  ...
  return (
    <main className="mx-auto max-w-7xl px-6 py-12">
      <div className="flex flex-wrap items-end justify-between gap-5">
        <div>
          <p className="text-xs uppercase tracking-[0.22em] text-brand">For creators</p>
          <h1 className="mt-2 text-4xl font-semibold">Wallet & earnings</h1>
          <p className="mt-3 max-w-3xl text-text-secondary">See what you sold, what BVS earned, the actual payment-processing deduction when connected, and what reached your payout balance.</p>
        </div>
        <div className="flex flex-wrap gap-2"><Link href="/upload" className="rounded-full border border-white/20 px-5 py-2 text-sm hover:border-brand">Submit music / beats</Link><Link href="/premium" className="rounded-full border border-brand/40 px-5 py-2 text-sm text-brand hover:bg-brand/10">Compare Premium</Link></div>
      </div>
      ...
      <section className="mt-10 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        <Metric label="Available balance" value={money(data.balances.available)} note="Posted credits minus posted debits" />
        <Metric label="Pending earnings" value={money(data.balances.pendingEarnings)} note="Waiting for processor reconciliation" warning={data.balances.pendingEarnings > 0} />
        <Metric label="Lifetime product sales" value={money(data.earnings.lifetimeGrossSales)} note="Pre-tax GMV attributed to you" />
        <Metric label="BVS platform fees" value={money(data.earnings.bvsPlatformFees)} note="Commission on pre-tax product price" />
        <Metric label="Processing deducted" value={money(data.earnings.processorFees)} note="Actual/schedule allocation; never hidden BVS revenue" />
      </section>

      {hasCommerce ? <section className="mt-8 rounded-2xl border border-brand/25 bg-brand/[.05] p-5"><h2 className="text-xl font-semibold">How a marketplace sale reaches your wallet</h2><div className="mt-4 flex flex-wrap items-center gap-2 text-sm"><FlowStep text="Product sale" /><Arrow /><FlowStep text="BVS platform fee" /><Arrow /><FlowStep text="Payment processing" /><Arrow /><FlowStep text="Your net earnings" /><Arrow /><FlowStep text="Available for payout" /></div><p className="mt-4 text-sm text-text-secondary">VAT/sales tax is excluded before the marketplace split. BVS commission is never calculated on customer VAT. A processor fee is deducted at the actual amount when available; otherwise the sale remains pending rather than using a hidden estimate.</p></section> : null}

      <section className="mt-12 rounded-2xl border border-white/10 bg-white/[0.025] p-6">
        <div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs uppercase tracking-[.16em] text-brand">Sale statements</p><h2 className="mt-1 text-2xl font-semibold">Marketplace earnings</h2></div><p className="text-xs text-text-secondary">Policy is snapshotted per sale; historical entries are not silently rewritten.</p></div>
        {data.sellerSettlements.length ? <div className="mt-5 overflow-x-auto"><table className="min-w-[900px] w-full text-left text-sm"><thead className="text-xs uppercase tracking-[.08em] text-text-secondary"><tr><th className="pb-3 pr-4">Sale</th><th className="pb-3 pr-4">Plan</th><th className="pb-3 pr-4">Gross</th><th className="pb-3 pr-4">BVS fee</th><th className="pb-3 pr-4">Processing</th><th className="pb-3 pr-4">You earned</th><th className="pb-3 pr-4">Status</th><th className="pb-3">Policy</th></tr></thead><tbody>{data.sellerSettlements.map((settlement) => { const firstLine = settlement.breakdown?.lines?.[0]; return <tr key={settlement.id} className="border-t border-white/10"><td className="py-4 pr-4"><div className="font-medium">{firstLine?.title || settlement.order_reference}</div><div className="text-xs text-text-secondary">{settlement.order_reference} · {settlement.provider}</div></td><td className="py-4 pr-4 text-text-secondary">{(settlement.seller_plan_id || 'free').replaceAll('_', ' ')}</td><td className="py-4 pr-4">{money(settlement.gross_product_revenue)}</td><td className="py-4 pr-4">{money(settlement.platform_fee_amount)}<div className="text-xs text-text-secondary">{pctFromBps(settlement.platform_fee_bps)}</div></td><td className="py-4 pr-4">{money(settlement.processor_fee_allocated)}<div className="text-xs capitalize text-text-secondary">{settlement.processor_fee_status.replaceAll('_', ' ')}</div></td><td className="py-4 pr-4 font-semibold text-brand">{money(settlement.seller_net)}</td><td className={`py-4 pr-4 capitalize ${settlement.settlement_status === 'posted' ? 'text-emerald-300' : settlement.settlement_status === 'reversed' ? 'text-red-300' : 'text-amber-200'}`}>{settlement.settlement_status.replaceAll('_', ' ')}</td><td className="py-4 text-xs text-text-secondary">{settlement.policy_version}</td></tr> })}</tbody></table></div> : <p className="mt-5 rounded-xl border border-dashed border-white/15 p-6 text-sm text-text-secondary">No marketplace sale statements yet. Once a verified sale is attributed to you, its BVS fee, processor deduction and net earnings will appear here.</p>}
      </section>
      ...
      <section className="mt-12 grid gap-8 lg:grid-cols-[1.05fr_0.95fr]">
        <form onSubmit={submitWaitlist} className="rounded-2xl border border-white/10 bg-white/[0.025] p-6">
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            ...
            <label className="text-sm font-medium sm:col-span-2">Notes<textarea value={form.notes} onChange={(event) => setForm({ ...form, notes: event.target.value })} rows={4} className="mt-1.5 w-full resize-y rounded-xl border border-white/10 bg-black/20 p-3 outline-none focus:border-brand" /></label>
          </div>
          <button disabled={saving || !form.artistName} className="mt-6 rounded-full bg-brand px-6 py-3 font-semibold text-black disabled:opacity-60">{saving ? 'Saving...' : data.waitlist ? 'Update queue entry' : 'Join artist queue'}</button>
        </form>

        <div className="space-y-6">
          <section className="rounded-2xl border border-white/10 bg-white/[0.025] p-6"><h2 className="text-2xl font-semibold">Creditable deposit</h2><p className="mt-2 text-sm text-text-secondary">Reserve priority onboarding/review access. The deposit is recorded as BVS platform credit after payment is confirmed.</p><div className="mt-5 flex flex-wrap gap-3">{[5, 10, 20].map((amount) => <button key={amount} onClick={() => reserveDeposit(amount)} className="rounded-full border border-brand px-5 py-2 text-sm font-semibold text-brand hover:bg-brand hover:text-black">{money(amount)}</button>)}</div></section>
          <section className="rounded-2xl border border-white/10 bg-white/[0.025] p-6"><h2 className="text-2xl font-semibold">Payout readiness</h2><p className="mt-2 text-sm text-text-secondary">{nextPayout > 0 ? `${money(nextPayout)} more in posted balance is needed before payout requests open.` : 'Posted balance is above the current payout threshold.'}</p><p className="mt-3 text-xs text-text-secondary">Pending processor settlements do not count toward payout availability. Refunds and chargebacks appear as separate debits so the original sale history remains intact.</p></section>
        </div>
      </section>

      <section className="mt-12 grid gap-8 lg:grid-cols-2">
        <ActivityTable title="Wallet ledger" rows={data.ledger.map((entry) => [entry.entry_type.replaceAll('_', ' '), entry.status, `${entry.direction === 'debit' ? '−' : '+'}${money(entry.amount, entry.currency)}`, new Date(entry.effective_at).toLocaleDateString()])} empty="No wallet entries yet." />
...
function Metric({ label, value, note, warning = false }: { label: string; value: string; note?: string; warning?: boolean }) {
  return <div className={`rounded-2xl border p-5 ${warning ? 'border-amber-400/30 bg-amber-400/[.06]' : 'border-white/10 bg-white/[0.03]'}`}><p className="text-sm text-text-secondary">{label}</p><p className={`mt-2 text-2xl font-semibold capitalize ${warning ? 'text-amber-200' : 'text-brand'}`}>{value}</p>{note ? <p className="mt-2 text-xs text-text-secondary">{note}</p> : null}</div>
}
