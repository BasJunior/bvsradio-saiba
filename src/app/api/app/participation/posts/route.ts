...
async function hydratePosts(threads: ThreadRow[], viewerId: string | null): Promise<ParticipationPost[]> {
  ...
  return threads.flatMap((thread) => {
    ...
    const attachment: ParticipationTarget | null = thread.attachment_kind && thread.attachment_id && thread.attachment_title
      ? {
        kind: thread.attachment_kind,
        id: thread.attachment_id,
        title: thread.attachment_title,
...
export async function GET(request: Request) {
  ...
  const rows = await participationRows<ThreadRow>(
    `participation_threads?thread_type=eq.post&status=eq.published${cursorClause}&select=id,author_user_id,intent,attachment_kind,attachment_id,attachment_title,attachment_href,created_at,updated_at&order=created_at.desc,id.desc&limit=${Math.min(75, limit + blocked.size + 10)}`,
  );
  const visible = rows.filter((row) => !blocked.has(row.author_user_id)).slice(0, limit);
  return NextResponse.json({
    enabled: true,
    posts: await hydratePosts(visible, user?.id || null),
...
export async function POST(request: Request) {
  ...
  if (body.attachment?.kind || body.attachment?.id) {
    ...
    attachment = await resolveParticipationTarget(kind, id, parseSurface(body.surface));
    if (!attachment) return NextResponse.json({ error: "That attachment is not public or is no longer eligible." }, { status: 404 });
  }

  const requestedMentionIds = [...new Set((body.mentionUserIds || []).map(String).filter(Boolean))].slice(0, 5);
  const eligibleMentions = requestedMentionIds.length ? await loadParticipationProfiles(requestedMentionIds) : [];
  ...
  const posts = await hydratePosts(threadRows, user.id);
