...
export async function GET(request: Request) {
  ...
  try {
    const [
      ...
      optionalRows(`commerce_refund_events?created_at=gte.${encodeURIComponent(quarterStart)}&select=id,provider,provider_event_id,order_id,order_reference,event_type,provider_amount,provider_currency,reversal_fraction,created_at&order=created_at.desc&limit=1000`),
      optionalRows('marketplace_fee_policy_audit?select=id,policy_version,actor_user_id,action,details,created_at&order=created_at.desc&limit=50'),
    ...
    const quarterRefundDebits = quarterLedger.rows
      .filter((row) => row.direction === 'debit' && ['refund_debit', 'reversal_debit'].includes(String(row.entry_type)))
      .reduce((sum, row) => sum + money(row.amount), 0)
    ...
    const actualOrderProcessorFees = uniqueOrderProcessorFees(
      settlements.rows.filter((row) => row.processor_fee_status === 'actual' || row.processor_fee_status === 'schedule'),
    )
    ...
    const marketplaceExamples = [
      { id: 'single_2', label: '$2 single · Artist Free', productType: 'single', price: 2, sellerPlanId: 'artist_free', revenueModel: 'marketplace' },
      { id: 'track_4', label: '$4 track / archive / mix · Artist Free', productType: 'mix', price: 4, sellerPlanId: 'artist_free', revenueModel: 'marketplace' },
      { id: 'track_4_premium', label: '$4 track · Artist Premium', productType: 'mix', price: 4, sellerPlanId: 'artist_standard', revenueModel: 'marketplace' },
      { id: 'album_14', label: '$14 album', productType: 'album', price: 14, sellerPlanId: 'artist_free', revenueModel: 'marketplace' },
      { id: 'album_19', label: '$19 album', productType: 'album', price: 19, sellerPlanId: 'artist_free', revenueModel: 'marketplace' },
      { id: 'beat_free', label: '$29 beat · Producer Free', productType: 'beat', price: 29, sellerPlanId: 'producer_free', revenueModel: 'marketplace' },
      { id: 'beat_plus', label: '$29 beat · Producer Plus', productType: 'beat', price: 29, sellerPlanId: 'producer_plus', revenueModel: 'marketplace' },
      { id: 'beat_pro', label: '$29 beat · Producer Pro', productType: 'beat', price: 29, sellerPlanId: 'producer_pro', revenueModel: 'marketplace' },
      { id: 'service_free', label: '$69 service · Free provider', productType: 'service', price: 69, sellerPlanId: 'service_free', revenueModel: 'marketplace' },
      { id: 'service_pro', label: '$69 service · Service Pro', productType: 'service', price: 69, sellerPlanId: 'service_pro', revenueModel: 'marketplace' },
      { id: 'studio', label: '$69 service · Studio', productType: 'service', price: 69, sellerPlanId: 'studio', revenueModel: 'marketplace' },
    ].map((item) => ({
      ...item,
      commissionBps: marketplaceCommissionBps({
        productType: item.productType as 'single' | 'mix' | 'album' | 'beat' | 'service',
    ...
    const feeTable = [
      { client: 'Artist Free', subscription: '$0', product: 'Low-ticket BVS music', platformFee: '20%', processing: 'Separate from creator proceeds' },
      { client: 'Artist Premium', subscription: '$12/mo · $120/yr', product: 'Eligible BVS music', platformFee: '15%', processing: 'Separate from creator proceeds' },
      { client: 'Producer Free', subscription: '$0', product: 'Beat licences', platformFee: '15%', processing: 'Separate from producer proceeds' },
      { client: 'Producer Plus', subscription: '$5/mo · $50/yr', product: 'Beat licences', platformFee: '8%', processing: 'Separate from producer proceeds' },
      { client: 'Producer Pro', subscription: '$10/mo · $100/yr', product: 'Beat licences', platformFee: '3%', processing: 'Separate from producer proceeds' },
      { client: 'Creator Complete', subscription: '$19/mo · $190/yr · later', product: 'Beats / eligible music', platformFee: '3% beats · 15% music', processing: 'Separate from creator proceeds' },
      { client: 'Service Free', subscription: '$0 · later', product: 'Marketplace services', platformFee: '15%', processing: 'Separate from provider proceeds' },
      { client: 'Service Pro', subscription: '$8/mo · $80/yr · later', product: 'Marketplace services', platformFee: '8%', processing: 'Separate from provider proceeds' },
      { client: 'Studio', subscription: '$15/mo · $150/yr · later', product: 'Marketplace services', platformFee: '5%', processing: 'Separate from provider proceeds' },
      { client: 'BVS Supporter', subscription: '$3/mo · $30/yr · pilot', product: 'Station support', platformFee: 'N/A', processing: 'BVS subscription cost' },
      { client: 'Team / Label', subscription: 'Invite pilot only', product: 'Roster tools', platformFee: 'Validate', processing: 'Do not lock before unit economics' },
    ]
    ...
    return NextResponse.json({
      ...
      accounting: {
        ...
        quarterRefundDebits: quarterLedger.available ? quarterRefundDebits : null,
      ...
      recentSettlements,
      recentRefundEvents: refundEvents.available ? refundEvents.rows.slice(0, 50) : [],
      caseStudy: {
