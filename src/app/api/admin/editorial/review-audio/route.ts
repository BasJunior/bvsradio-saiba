...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (kind === 'track') {
      ...
      // Canonical privileged resolver remains master-only. The Editorial player
      // may use its separately server-signed submitted preview only after this
      // endpoint explicitly reports that no master is attached.
