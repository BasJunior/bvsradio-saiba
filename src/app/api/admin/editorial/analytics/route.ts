...
export async function GET(request: Request) {
  ...
  try {
    ...
    const eventTrackId = (event: AnalyticsEvent) => {
      const rawId = String(event.properties?.track_id || '')
      return legacyTrackIds.get(rawId) || rawId
    }
    const countEvent = (name: string) => events.filter((event) => event.event_name === name).length
    const playbackStarts = events.filter((event) => event.event_name === 'player_start' && trackName.has(eventTrackId(event)))
    const uniqueSessions = new Set(playbackStarts.map((event) => event.session_id).filter(Boolean)).size
    ...
    const topSaved = Object.entries(savedByTrack)
      ...
      .slice(0, 5)
    const playedByTrack = playbackStarts.reduce<Record<string, number>>((acc, event) => {
      const id = eventTrackId(event)
      if (trackName.has(id)) acc[id] = (acc[id] || 0) + 1
      return acc
    }, {})
    const topPlayed = [...tracks]
      .filter((track) => Number(playedByTrack[String(track.id)] || 0) > 0)
      .sort((a, b) => Number(playedByTrack[String(b.id)] || 0) - Number(playedByTrack[String(a.id)] || 0))
      .slice(0, 5)
      .map((track) => ({
        id: String(track.id),
        label: `${track.title} — ${track.artist_name}`,
        plays: Number(playedByTrack[String(track.id)] || 0),
      }))
    const popularGenres = tracks.reduce<Record<string, number>>((acc, track) => {
      const genre = String(track.genre || 'Unspecified')
      acc[genre] = (acc[genre] || 0) + Number(playedByTrack[String(track.id)] || 0)
      return acc
    }, {})
