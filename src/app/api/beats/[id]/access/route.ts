...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!url || !service) return NextResponse.json({ owned: false })

  const token = request.headers.get('authorization')?.replace(/^Bearer\s+/i, '').trim()
  if (!token) return NextResponse.json({ owned: false })
  const user = await authUserId(url, service, token)
  if (!user?.id) return NextResponse.json({ owned: false })
  ...
  if (!ordersResponse.ok) return NextResponse.json({ owned: false })
  ...
  if (!order || !item) return NextResponse.json({ owned: false })

  const [beatResponse, workspaceResponse] = await Promise.all([
    fetch(`${url}/rest/v1/beats?id=eq.${encodeURIComponent(beatId)}&select=id,master_path,preview_path&limit=1`, { headers, cache: 'no-store' }),
    fetch(`${url}/rest/v1/song_workspaces?user_id=eq.${encodeURIComponent(user.id)}&order_id=eq.${encodeURIComponent(order.id)}&beat_id=eq.${encodeURIComponent(beatId)}&select=id&limit=1`, { headers, cache: 'no-store' }),
  ])
  ...
  if (!beat) return NextResponse.json({ owned: false })

  const fullAudioUrl = await signedAudio(beat.master_path || beat.preview_path)
