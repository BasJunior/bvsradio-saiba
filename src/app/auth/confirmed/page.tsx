...
function friendlyAuthError(code: string | null, description: string | null): string {
  ...
  if (code === 'otp_expired' || /expired|invalid/i.test(desc)) {
    return 'This confirmation link is invalid or has already been used. Inbox scanners can open one-time links before you do. Request a fresh confirmation, then open the newest email in a full browser/app view.'
  }
  ...
  useEffect(() => {
    const finishConfirmation = async () => {
      try {
        ...
        setStatus('ready')
        if (/^\/app\/(ios|android)(?:\/|$)/.test(next)) {
          window.setTimeout(() => window.location.replace(next), 850)
        }
      } catch (err: unknown) {
  ...
  return (
    <main className="mx-auto flex min-h-[70vh] max-w-xl items-center px-6 py-16 text-center">
      <div className="w-full rounded-3xl border border-white/10 bg-bg-card/50 p-8">
        ...
        {status === 'ready' && <><p className="text-xs uppercase tracking-[.2em] text-brand">Email confirmed</p><h1 className="mt-3 text-3xl font-semibold">Welcome to BVS Radio</h1><p className="mt-3 text-text-secondary">Your account is ready and you are signed in.{nextPath.startsWith('/app/') ? ' Returning you to the app…' : ''}</p><Link href={nextPath} className="mt-7 inline-block rounded-full bg-brand px-7 py-3 font-semibold text-black">Continue to BVS</Link></>}
        {status === 'error' && <><h1 className="text-3xl font-semibold">We could not confirm this link</h1><p className="mt-3 text-left text-text-secondary">{detail}</p><ol className="mt-4 list-decimal space-y-2 px-4 text-left text-sm text-text-secondary"><li>Request a fresh confirmation link.</li><li>Use the newest email only.</li><li>Open the link in a full browser/app view, not a mail preview.</li></ol><div className="mt-7 flex flex-wrap justify-center gap-3"><Link href={`/auth/signup${nextPath !== '/' ? `?next=${encodeURIComponent(nextPath)}` : ''}`} className="rounded-full bg-brand px-6 py-3 font-semibold text-black">Resend confirmation</Link><Link href={`/auth/login${nextPath !== '/' ? `?next=${encodeURIComponent(nextPath)}` : ''}`} className="rounded-full border border-white/20 px-6 py-3">Sign in</Link></div></>}
      </div>
    </main>
  )
}
