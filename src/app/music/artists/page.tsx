...
export default async function ArtistsDirectoryPage() {
  ...
  return (
    <main className="mx-auto min-h-[70vh] max-w-6xl px-6 pb-12 pt-8 sm:pt-10">
      ...
      <h1 className="mt-2 scroll-mt-28 text-4xl font-semibold md:text-5xl">Published artists</h1>
      ...
      {artists.length ? (
        <div className="mt-10 grid grid-cols-2 gap-x-5 gap-y-8 md:grid-cols-3 lg:grid-cols-4">
          {artists.map((artist) => (
            <Link key={artist.id} href={`/artist/${artist.username}`} className="group min-w-0 text-center">
