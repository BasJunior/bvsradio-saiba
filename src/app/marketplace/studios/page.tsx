...
import { haversineKm, studioPriceLabel, type StudioDiscoveryProfile } from "@/lib/studio-marketplace";
...
function nextSlotLabel(value?: string | null) {
  if (!value) return "View availability";
  const date = new Date(value);
  if (!Number.isFinite(date.getTime())) return "View availability";
  return `Next slot ${new Intl.DateTimeFormat("en-GB", { weekday: "short", day: "numeric", month: "short" }).format(date)}`;
}
...
export default function StudioMarketplacePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([marketplaceData, studioData]) => {
        ...
        const firstCity = studioData.cities?.[0]?.city || studioData.studios?.[0]?.city || "";
        setSelectedCity((current) => current || firstCity);
  ...
  }, [cards, selectedCity, cityQuery, serviceFilter]);
    ...
    <main className="mx-auto max-w-[1500px] px-4 pb-16 pt-8 sm:px-6 lg:px-8">
      <section className="overflow-hidden rounded-[2rem] border border-white/10 bg-[radial-gradient(circle_at_20%_10%,rgba(38,181,145,.15),transparent_32%),linear-gradient(135deg,rgba(255,255,255,.04),rgba(255,255,255,.015))] p-6 sm:p-9">
        <div className="max-w-4xl">
          ...
          <p className="mt-4 max-w-3xl text-base text-text-secondary sm:text-lg">Find BVS studios by city, compare real packages and prices, check verified client ratings, then choose published availability without leaving the BVS creator ecosystem.</p>
        </div>

        <div className="mt-7 grid gap-3 rounded-2xl border border-white/10 bg-black/20 p-3 md:grid-cols-[minmax(0,1fr)_auto]">
          <label className="flex items-center gap-3 rounded-xl bg-white/[.04] px-4">
            <span aria-hidden>⌖</span>
            <input
              value={cityQuery}
              onChange={(event) => { setCityQuery(event.target.value); setSelectedCity(""); }}
              placeholder="Search Harare, Bulawayo, Johannesburg…"
              className="min-h-12 w-full bg-transparent text-sm outline-none placeholder:text-white/35"
            />
          </label>
          <button type="button" onClick={useMyLocation} className="min-h-12 rounded-xl bg-brand px-5 text-sm font-semibold text-black">Use my location</button>
        </div>
        {locationMessage ? <p className="mt-3 text-xs text-text-secondary">{locationMessage}</p> : null}

        <div className="mt-5 flex gap-2 overflow-x-auto pb-1">
          {(studioPayload.cities || []).map((city) => (
            <button
              key={`${city.city}-${city.country}`}
              type="button"
              onClick={() => { setSelectedCity(city.city); setCityQuery(city.city); }}
              className={`shrink-0 rounded-full border px-4 py-2 text-sm ${selectedCity === city.city ? "border-brand bg-brand/10 text-brand" : "border-white/10 text-text-secondary hover:border-white/25"}`}
            >
      ...
      <section className="mt-7 flex flex-wrap items-center justify-between gap-4">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[.16em] text-brand">Studio discovery</p>
          <h2 className="mt-1 text-2xl font-semibold sm:text-3xl">{cityCount} {cityCount === 1 ? "BVS studio" : "BVS studios"} in {activeCity}</h2>
        </div>
        <div className="flex flex-wrap gap-2">
          {[
            ...
            <button key={value} type="button" onClick={() => setServiceFilter(value)} className={`rounded-full border px-3 py-2 text-xs ${serviceFilter === value ? "border-brand bg-brand/10 text-brand" : "border-white/10 text-text-secondary"}`}>{label}</button>
          ))}
        </div>
      </section>

      <div className="mt-5 flex rounded-full border border-white/10 p-1 lg:hidden">
        <button type="button" onClick={() => setMobileMode("list")} className={`flex-1 rounded-full py-2 text-sm ${mobileMode === "list" ? "bg-white/10" : "text-text-secondary"}`}>List</button>
        <button type="button" onClick={() => setMobileMode("map")} className={`flex-1 rounded-full py-2 text-sm ${mobileMode === "map" ? "bg-white/10" : "text-text-secondary"}`}>Map</button>
      </div>
      ...
      {state === "ready" ? (
        <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,760px)_minmax(420px,1fr)]">
          <div className={mobileMode === "map" ? "hidden lg:block" : "space-y-4"}>
            {!filtered.length ? (
              <div className="rounded-[2rem] border border-dashed border-white/15 p-10 text-center">
                <h3 className="text-xl font-semibold">No published BVS studios match this search yet.</h3>
                <p className="mt-2 text-sm text-text-secondary">Try another city or remove the service filter. BVS never invents studio availability or ratings.</p>
              </div>
            ) : null}
            {filtered.map(({ studio, provider, distanceKm }) => {
              ...
              return (
                <article
                  ...
                  onMouseEnter={() => setSelectedKey(studio.providerKey)}
                  className={`group overflow-hidden rounded-[1.75rem] border bg-white/[.025] transition ${selectedKey === studio.providerKey ? "border-brand/50 shadow-xl shadow-brand/5" : "border-white/10 hover:border-white/20"}`}
                >
                  <div className="grid sm:grid-cols-[240px_minmax(0,1fr)]">
                    <Link href={`/marketplace/studios/${studio.providerKey}`} className="relative block min-h-52 overflow-hidden bg-black/30">
                      {image ? <img src={image} alt="" className="absolute inset-0 h-full w-full object-cover transition duration-500 group-hover:scale-[1.03]" /> : null}
                      <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent sm:bg-gradient-to-r" />
                      {studio.verified ? <span className="absolute left-3 top-3 rounded-full bg-brand px-3 py-1 text-[10px] font-bold uppercase tracking-[.12em] text-black">BVS verified</span> : null}
                    </Link>
                    <div className="p-5 sm:p-6">
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          <p className="text-xs text-text-secondary">{studio.locationLabel}{distanceKm != null ? ` · ${distanceKm.toFixed(distanceKm < 10 ? 1 : 0)} km away` : ""}</p>
                          <Link href={`/marketplace/studios/${studio.providerKey}`} className="mt-1 block text-2xl font-semibold hover:text-brand">{studio.displayName}</Link>
                        </div>
                        <div className="text-right">
                          {studio.rating ? <p className="font-semibold">★ {studio.rating.toFixed(1)} <span className="text-xs font-normal text-text-secondary">({studio.reviewCount})</span></p> : <p className="text-xs text-text-secondary">New on BVS</p>}
                          <p className="mt-1 text-sm font-semibold text-brand">{studioPriceLabel(studio.hourlyFromUsd)}</p>
                        </div>
                      </div>
                      <p className="mt-3 line-clamp-2 text-sm text-text-secondary">{provider.headline}</p>
                      <div className="mt-4 flex flex-wrap gap-2">
                        {provider.services.filter((service) => service.bookingMode === "calendar").slice(0, 3).map((service) => <span key={service.id} className="rounded-full bg-white/5 px-3 py-1 text-xs text-text-secondary">{service.title}</span>)}
                      </div>
                      <div className="mt-5 flex flex-wrap items-center justify-between gap-3">
                        <span className="text-xs text-text-secondary">{nextSlotLabel(studio.nextAvailableAt)}</span>
                        <Link href={`/marketplace/studios/${studio.providerKey}`} className="rounded-full bg-brand px-4 py-2 text-sm font-semibold text-black">View studio</Link>
                      </div>
                    </div>
                  </div>
                </article>
          ...
          <aside className={`${mobileMode === "list" ? "hidden lg:block" : "block"} h-fit lg:sticky lg:top-24`}>
            <StudioDiscoveryMap studios={filtered.map((item) => item.studio)} selectedKey={selectedKey} onSelect={setSelectedKey} />
          </aside>
        </div>
      ) : null}
    </main>
