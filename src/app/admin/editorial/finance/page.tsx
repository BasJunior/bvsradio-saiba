  ...
  const processorMatrix = useMemo(() => {
    ...
    return data.policy.processorPresets.map((preset) => {
      ...
      return {
        ...preset,
        fee,
        sellerPays: calculateMarketplaceEconomics({ productPrice: example.price, taxRatePercent: taxRate, commissionBps, processorFee: fee, processorAllocatedToSeller: fee }),
        bvsAbsorbs: calculateMarketplaceEconomics({ productPrice: example.price, taxRatePercent: taxRate, commissionBps, processorFee: fee, processorAllocatedToSeller: 0 }),
      }
    })
...
function Line({ label, value, strong = false }: { label: string; value: string; strong?: boolean }) { return <div className={`flex justify-between gap-4 ${strong ? 'border-t border-white/10 pt-2 font-semibold' : 'text-text-secondary'}`}><span>{label}</span><span className={strong ? 'text-brand' : 'text-text-primary'}>{value}</span></div> }
function Input({ value, onChange, placeholder, type = 'text' }: { value: string; onChange: (value: string) => void; placeholder: string; type?: string }) { return <input value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} type={type} min={type === 'number' ? '0' : undefined} step={type === 'number' ? '0.01' : undefined} className="rounded-xl border border-white/10 bg-background px-3 py-2 text-sm" /> }
